...
|--------|---------|----------|
| `[AI]` | {{EXECUTOR_AI}} executed by AI | You (AI) |
| `[USER]` | {{USER_ACTION_LABEL}} executed by user | User |
...
2. If code was changed, remind user to run `{{CMD_REF:finish-work}}`
...
See {{CMD_REF:brainstorm}} for the full process. Summary:
...
> See {{CMD_REF:brainstorm}} Step 8 for details.
...
Based on the confirmed PRD, run a research pass to find relevant specs and patterns:
...
Add spec files found in the research pass:
...
Task(
  ...
  Run project checks (lint, type-check, tests) before finishing.",
  model: "opus"
)
```
{{/AGENT_CAPABLE}}
...
Task(
  ...
  Ensure project checks pass (lint, type-check, tests).",
  model: "opus"
)
```
{{/AGENT_CAPABLE}}
...
1. Verify project checks pass
2. Report what was implemented
3. Remind user to:
   - Test the changes
   - Commit when ready
   - Run `{{CMD_REF:finish-work}}` to record this session
...
### User {{USER_ACTION_LABEL}} `[USER]`

| Name | When to Use |
|------|-------------|
| `{{CMD_REF:start}}` | Begin a session |
| `{{CMD_REF:finish-work}}` | Before committing + record session |

### AI Scripts `[AI]`

| Script | Purpose |
|--------|---------|
| `{{PYTHON_CMD}} ./.trellis/scripts/get_context.py` | Get session context |
| `{{PYTHON_CMD}} ./.trellis/scripts/task.py create` | Create task directory |
| `{{PYTHON_CMD}} ./.trellis/scripts/task.py init-context` | Initialize jsonl files |
| `{{PYTHON_CMD}} ./.trellis/scripts/task.py add-context` | Add spec/context file to jsonl |
| `{{PYTHON_CMD}} ./.trellis/scripts/task.py start` | Set current task |
| `{{PYTHON_CMD}} ./.trellis/scripts/task.py finish` | Clear current task |
| `{{PYTHON_CMD}} ./.trellis/scripts/task.py archive` | Archive completed task |

{{#AGENT_CAPABLE}}
### Sub Agents `[AI]`

| Agent | Purpose | Context Source |
|-------|---------|----------------|
...
>
> The Task Workflow ensures relevant spec context is loaded automatically.
